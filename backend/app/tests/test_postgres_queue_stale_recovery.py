...
def test_recover_stale_jobs_does_not_affect_active_leases():
    ...
    try:
        ...
        job.locked_at = datetime.now(timezone.utc)
        job.lease_expires_at = datetime.now(timezone.utc).timestamp() + 300  # Active
        job.state = "running"
        db.commit()
    finally:
        db.close()
    ...
    try:
        ...
        assert job.lease_expires_at > datetime.now(timezone.utc).timestamp()
    finally:
        db.close()
...
def test_recover_stale_jobs_increments_retry_count():
    ...
    try:
        ...
        job.locked_at = datetime.now(timezone.utc)
        job.lease_expires_at = datetime.now(timezone.utc).timestamp() - 100
        job.state = "running"
        job.retry_count = 2
        db.commit()
    finally:
