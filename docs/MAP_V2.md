# MAP V2 — MapLibre Implementation

## Overview

`/map-v2` is a parallel map route built on [MapLibre GL JS](https://maplibre.org/) 4.x.
It replaces the Leaflet prototype at `/map` for future development while keeping the old
route fully intact.  Tiles are served by [OpenFreeMap](https://openfreemap.org/) — no API
key required.

## Route

```
/app/map-v2/page.tsx          — Next.js server page (metadata export)
/app/map-v2/MapV2Workspace.tsx — "use client" workspace component
```
...
```
MapV2Page (server)
└── MapV2Workspace (client — owns fetch state)
