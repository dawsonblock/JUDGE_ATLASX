...
def main() -> int:
    ...
    if args.dry_run:
        # Perform same validation as real build
        release_gate = _load_release_gate(REPO_ROOT)
        packaged_proof_paths = _load_packaged_proof_paths(REPO_ROOT)
        proof_manifest_paths = _collect_proof_manifest_paths(REPO_ROOT)
        required_log_index_paths, required_index_exists_true_missing = (
        ...
        result = {
            "dry_run": True,
            "valid": (
                not missing_required_proof_files
            ...
            "missing_referenced_logs": missing_referenced_proof_paths,
        ...
        if not result["valid"]:
            if args.json:
                ...
                print("[dry-run] FAIL: Proof files are missing")
                if missing_required_proof_files:
                    print("  Missing required proof files:")
                    for f in missing_required_proof_files:
                        print(f"    {f}")
                if missing_referenced_proof_paths:
                    print("  Missing referenced proof logs:")
                    for f in missing_referenced_proof_paths:
                        print(f"    {f}")
            return 1
        ...
        result["root_name"] = args.root_name
        result["file_count"] = len(files)
        result["included_top_level_paths"] = sorted(included_top_level)
        result["excluded_top_level_paths"] = sorted(excluded_top_level)
        result["files"] = [_normalize(f.relative_to(REPO_ROOT)) for f in files]
